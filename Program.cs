...
namespace AccountManagement
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            //builder.Services.AddRazorPages(options=> options.Conventions.A);  // remove later as it might not be needed
            WebApplication app = builder.Build();

            
            // requirement from A2 (1) - data ID, firstname, lastname and email address < -- use json 
            app.MapPost("/account/register" , async (HttpContext context) => 
            {
                // Read the JSON body
                ...
                {
                    Console.WriteLine($"{property.Name} :  {property.Value}  -- string input validation " +
                                      $"{InputValidation.IsString(property.Value.ToString())}");
                }
                ...
                return Results.Ok();
            });  // validate data   id - int, string for the rest and email address should be the right format 

            // requirement from A2 (2) - retrieve  data using either the id or the email address
            app.MapGet("/account/search/id/{id}", (int id)=>"search using id number"); // validate int 
            app.MapGet("/account/search/email/{email}", (string email)=>
                {
                    WriteLine($"search using email address {email}"); 
                    WriteLine($"is email valid? {InputValidation.IsValidEmail(email)}"); 


                    });  // validate string name   // validate if the email format is correct and if the record exists - later on  
